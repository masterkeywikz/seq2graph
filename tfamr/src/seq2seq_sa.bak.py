...
def attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    def attention(query):
      ...
      masks = []  # Results of attention masks
      ...
      for a in xrange(num_heads):
        with variable_scope.variable_scope("Attention_%d" % a):
          ...
          masks.append(array_ops.reshape(a, [-1, attn_length]))
      return ds, masks
...
def sequence_loss_by_example(logits, targets, hard_aligns, weights,
  ...
  """
  if len(targets) != len(logits) or len(weights) != len(logits) or len(hard_aligns) != len(logits):
    raise ValueError("Lengths of logits, weights, hard_aligns and targets must be the same "
                     "%d, %d, %d, %d." % (len(logits), len(weights), len(hard_aligns), len(targets)))
  with ops.name_scope(name, "sequence_loss_by_example",
                      logits + targets + hard_aligns + weights):
    log_perp_list = []
...
def sequence_loss(logits, targets, hard_aligns, weights,
      ...
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        if per_example_loss:
          losses.append(sequence_loss_by_example(
              outputs[-1], targets[:bucket[1]], hard_aligns, weights[:bucket[1]],
              softmax_loss_function=softmax_loss_function))
        else:
