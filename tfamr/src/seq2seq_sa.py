...
def attention_decoder(decoder_inputs, initial_state, attention_states, cell,
  ...
  with variable_scope.variable_scope(scope or "attention_decoder"):
    ...
    def attention(query):
      ...
      masks = []  # Results of attention masks unnormalized
      for a in xrange(num_heads):
        with variable_scope.variable_scope("Attention_%d" % a):
          ...
          masks.append(array_ops.reshape(s, [-1, attn_length]))
      return ds, masks
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      if loop_function is not None:
        prev = output
      outputs.append(output)
      soft_aligns.append(masks[0])

  return outputs, soft_aligns, state
...
def sequence_loss_by_example(logits, targets, soft_aligns, hard_aligns, weights,
  ...
  Args:
    ...
    loss_lambda: hyperparameter to balance sequence loss and alignment loss
  ...
  """
  if len(targets) != len(logits) or len(weights) != len(logits) or len(hard_aligns) != len(soft_aligns) \
    or len(soft_aligns) != len(logits):
    raise ValueError("Lengths of logits, weights, soft_aligns, hard_aligns and targets must be the same "
                     "%d, %d, %d, %d, %d." % (len(logits), len(weights), len(soft_aligns), len(hard_aligns), len(targets)))
  with ops.op_scope(logits + targets + weights + soft_aligns + hard_aligns, name,
                    "sequence_loss_by_example"):
    log_perp_list = []
    attn_crossent_list = []
    for logit, target, weight, soft_align, hard_align in zip(logits, targets, weights, soft_aligns, hard_aligns):
      ...
      attn_crossent = nn_ops.softmax_cross_entropy_with_logits(soft_align, hard_align)
      attn_crossent_list.append(attn_crossent * weight)
      log_perp_list.append(crossent * weight)
    attn_crossents = math_ops.add_n(attn_crossent_list)
...
def sequence_loss(logits, targets, soft_aligns, hard_aligns, weights,
  ...
  Raises:
    ValueError: If len(logits) is different from len(targets) or len(weights).
  """
  with ops.op_scope(logits + targets + weights + soft_aligns + hard_aligns, name, "sequence_loss"):
    cost = math_ops.reduce_sum(sequence_loss_by_example(
        logits, targets, soft_aligns, hard_aligns, weights,
...
def model_with_buckets(encoder_inputs, decoder_inputs, hard_aligns, targets, weights,
  ...
  Args:
    ...
    hard_aligns: List of 2D Tensors of shape [batch_size x num_encoder_symbols] a.k.a the reference alignment.
  ...
  if len(hard_aligns) < buckets[-1][1]:
    raise ValueError("Length of hard alignments (%d) must be at least that of last"
                     "bucket (%d)." % (len(hard_aligns), buckets[-1][1]))
  if len(weights) < buckets[-1][1]:
    raise ValueError("Length of weights (%d) must be at least that of last"
                     "bucket (%d)." % (len(weights), buckets[-1][1]))

  all_inputs = encoder_inputs + decoder_inputs + hard_aligns + targets + weights
  losses = []
  outputs = []
  with ops.op_scope(all_inputs, name, "model_with_buckets"):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        bucket_outputs, soft_aligns,  _ = seq2seq(encoder_inputs[:bucket[0]],
                                    decoder_inputs[:bucket[1]])
        outputs.append(bucket_outputs)
        if per_example_loss:
          losses.append(sequence_loss_by_example(
              outputs[-1], targets[:bucket[1]], soft_aligns, hard_aligns[:bucket[1]], weights[:bucket[1]],
              softmax_loss_function=softmax_loss_function))
        else:
