...
class LSTMEncPlain(Recurrent):
    ...
    def transform(self, inputs):
        ...
        (out, cell), updates = self._scan(
            fn,
            [TT.dot(x, self.find('xh'))],
            [('h', batch_size), ('c', batch_size)])
        print updates.items()
        return dict(out=out, cell=cell), updates.items()


class LSTMDecPlain(Recurrent):
    ...
    def _scan_(self, fn, inputs, inits=None, name='scan'):
        outputs = []
        for i, x in enumerate(inits or inputs):
            if hasattr(x, 'shape'):
                x = self.initial_state(str(i), x.shape[1])
    ...
    def transform(self, inputs):
        ...
        cell_enc = inputs['hid1:cell'] # max_len * batch_size * emb_size
        # We only keep the last one. for each hid_enc
        mask = inputs['in:src_mask'] # this mask is different from that of dec.
        hid_enc_lst = hid_enc* mask
        cell_enc_lst = cell_enc * mask

        hid_enc_lst = TT.sum(hid_enc_lst, axis = 0)
        cell_enc_lst = TT.sum(cell_enc_lst, axis = 0)
       
        x = inputs['in:dst']

        batch_size = x.shape[1]
        (out, cell), updates = self._scan_(
            fn,
            #[TT.dot(x, self.find('xh')) + self.find('b')],
            [TT.dot(x, self.find('xh'))],
            [hid_enc_lst, cell_enc_lst]
            )
        updates_items = updates.items()
        return dict(out=out, cell=cell), updates_items
...
class LSTMDec(Recurrent):
    ...
    def setup(self):
        ...
        self.add_bias('cf', self.size)
        self.add_bias('co', self.size)
        self.add_weights('V', self.size, self.size)

    def _scan_(self, fn, inputs, inits=None, non_seq = None, name='scan'):
        outputs = []
        for i, x in enumerate(inits or inputs):
            if hasattr(x, 'shape'):
                x = self.initial_state(str(i), x.shape[1])
        ...
        outputs.append(None) # for alpha.
        outputs.append(None) # for att
        return theano.scan(
            fn,
            name=self._fmt(name),
            sequences=inputs,
